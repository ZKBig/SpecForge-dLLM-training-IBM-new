...
# headroom: the XPress loss runs ~76GB/80GB; expandable_segments avoids fragmentation OOM
...
export DEEPSPEC_WANDB_NAME=xpress-official-FULL-${VARIANT}-4node
...
OPTS=(--opts "exp_name=xpress8b_online_full_${VARIANT}")
if [[ "$VARIANT" == "conf0" ]]; then
    OPTS+=(--opts "model.confidence_head_alpha=0.0")
fi

CUDA_VISIBLE_DEVICES=0,1,2,3,4,5,6,7 python train.py \
    --config config/dspark/xpress_qwen3_8b_online_full.py \
    "${OPTS[@]}"
