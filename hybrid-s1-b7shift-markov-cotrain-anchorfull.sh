# The SHIFT-CONVENTION b7 Markov co-train arm -- and gate A of the shift-convention port.
#
...
#
# vs hybrid-s1-b7warm-markov-cotrain.sh (the fillin arm), the deltas:
#   ~ --block-convention shift        (the point of this arm)
#   ~ output-dir / wandb-name
# Everything else (loss alphas, gamma, rank 256, markov-only, lambda_base, lr, data) is kept.
#
#   bash submit.sh 2 xpress hybrid-s1-b7shift-markov-cotrain
...
torchrun --nnodes=$NNODES \
    ...
    --output-dir /proj/checkpoints/zwang619/hybrid_out/xpress-markov-b7shift-cotrain-anchorfull \
    --report-to wandb \
    --wandb-project ripple-dspark \
    --wandb-name xpress-markov-b7shift-cotrain-anchorfull \
    --resume \
